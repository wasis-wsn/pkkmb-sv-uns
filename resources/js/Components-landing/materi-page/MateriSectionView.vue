<template>
    <div>
        <!-- BUKU IMPLEMENTASI -->
        <section v-if="firstMateri" class="flex gap-5 self-center mt-[45px] max-md:flex-wrap max-md:mt-10"
            data-aos="fade-up">
            <div class="flex flex-col grow shrink-0 self-start px-5 text-center basis-0 w-fit max-md:max-w-full">
                ...
                <div class="justify-center items-center text-center">
                    <p v-for="(paragraph, index) in splitParagraphs(firstMateri.deskripsi_materi)" :key="index"
                        data-aos="fade-up"
                        class="z-20 text-stone-900 text-[18px] md:text-[24px] max-md:max-w-[100%] md:max-w-[100%]">
                        {{ paragraph }}
                    </p>
                </div>
            </div>
        ...
        <section v-if="firstMateri"
            ...
            <div
                class="relative z-20 flex justify-center mt-5 md:mt-36 px-8 pt-10 md:pt-20 mb-0 w-full bg-slate-100 max-w-[90%] sm:max-w-[80%] md:max-w-[70%] lg:max-w-[1100px] pb-10 rounded-[60px] md:px-20 md:pb-[100px]">
                <div class="w-full max-md:px-5 top-[50px]">
                    <div class="text-[16px] md:text-[20px]" data-aos="fade-up">
                        <ol class="list-decimal list-inside pl-5 space-y-4">
                            <li v-for="(paragraph, index) in splitParagraphs(firstMateri.isi_materi)" :key="index"
                                class="formatted-text text-justify">
                                {{ paragraph }}
                            </li>
                        </ol>
                    </div>
